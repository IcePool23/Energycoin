...
#if defined(NDEBUG)
# error "Nrgc cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Nrgc Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s NRGC additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    // Go zero after 107 micro-halvings (27 years) Mined 499 996 395 NRGC

    if (halvings >= 107)
        return 0;

    // Force block reward to 0.390625 after 96 halvings.
    if (halvings >= 96)
        return 0.390625 * COIN;       

    // Force block reward to 0.78125 after 80 halvings.
    if (halvings >= 80)
        return 0.78125 * COIN;

    // Force block reward to 1.5625 after 64 halvings.
    if (halvings >= 64)
        return 1.5625 * COIN;

    // Force block reward to 3.125 after 48 halvings.
    if (halvings >= 48)
        return 3.125 * COIN;

    // Force block reward to 6.25 after 32 halvings.
    if (halvings >= 32)
        return 6.25 * COIN;

    // Force block reward to 12.5 after 24 halvings.
    if (halvings >= 24)
        return 12.5 * COIN;

    // Force block reward to 25 after 16 halvings.
    if (halvings >= 16)
        return 25 * COIN;

    // Force block reward to 50 after 12 halvings.
    if (halvings >= 12)
        return 50 * COIN;
		
    // Force block reward to 100 after 8 halvings.
    if (halvings >= 8)
        return 100 * COIN;

    // Force block reward to 200 after 4 halvings.
    if (halvings >= 4)
        return 200 * COIN;
		
    // Force block reward to 367 after 3 halvings.
    if (halvings >= 3)
        return 367.2 * COIN;

    // Force block reward to 459 after 2 halvings.
    if (halvings >= 2)
        return 459 * COIN;
		
    // Force block reward to 540 after 1 halving.
    if (halvings >= 1)
        return 540 * COIN;

    // Force block reward to 600 for the first period.
    if (halvings >= 0)
        return 600 * COIN;
}
...
void UpdateCoins(const CTransaction& tx, CCoinsViewCache& inputs, CTxUndo &txundo, int nHeight, uint256 blockHash, CAssetsCache* assetCache, std::pair<std::string, CBlockAssetUndo>* undoAssetData)
{
    ...
    if (!tx.IsCoinBase()) {
        ...
        for (const CTxIn &txin : tx.vin) {
            ...
            bool is_spent = inputs.SpendCoin(txin.prevout, &txundo.vprevout.back(), assetCache); /** NRGC START */ /* Pass assetCache into function */ /** NRGC END */
            assert(is_spent);
        }
    }
    // add outputs
    AddCoins(inputs, tx, nHeight, blockHash, false, assetCache, undoAssetData); /** NRGC START */ /* Pass assetCache into function */ /** NRGC END */
}
...
//                                      << ", " << i << ", " << hash.GetHex() << ", " << k << ", " << "true" << ", " << assetAmount << std::endl;
        ...
        for (size_t o = 0; o < tx.vout.size(); o++) {
            if (!tx.vout[o].scriptPubKey.IsUnspendable()) {
                ...
                Coin coin;
                bool is_spent = view.SpendCoin(out, &coin, &tempCache); /** NRGC START */ /* Pass assetsCache into the SpendCoin function */ /** NRGC END */
        ...
        if (i > 0) { // not coinbases
            ...
            for (unsigned int j = tx.vin.size(); j-- > 0;) {
                ...
                int res = ApplyTxInUndo(std::move(undo), view, out, assetsCache); /** NRGC START */ /* Pass assetsCache into ApplyTxInUndo function */ /** NRGC END */
...
bool static FlushStateToDisk(const CChainParams& chainparams, CValidationState &state, FlushStateMode mode, int nManualPruneHeight) {
    ...
    try {
    {
        ...
        if (fDoFullFlush) {
            ...
            if (!CheckDiskSpace((48 * 2 * 2 * pcoinsTip->GetCacheSize()) + assetDirtyCacheSize * 2)) /** NRGC START */ /** NRGC END */
                return state.Error("out of disk space");
...
bool static ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
...

    {
        ...
        LogPrint(BCLog::BENCH, "  - Flush NRGC: %.2fms [%.2fs (%.2fms/blk)]\n", (nTime4 - nTime3) * MILLI, nTimeFlush * MICRO, nTimeFlush * MILLI / nBlocksTotal);
...
bool TestBlockValidity(CValidationState& state, const CChainParams& chainparams, const CBlock& block, CBlockIndex* pindexPrev, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (!ConnectBlock(block, state, &indexDummy, viewNew, chainparams, &assetCache, true)) /** NRGC START */ /*Add asset to function */ /** NRGC END*/
