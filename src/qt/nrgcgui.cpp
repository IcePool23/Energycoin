...
const std::string NrgcGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString NrgcGUI::DEFAULT_WALLET = "~Default";
...
static bool ThreadSafeMessageBox(NrgcGUI *gui, const std::string& message, const std::string& caption, unsigned int style);

NrgcGUI::NrgcGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
...
void NrgcGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Nrgc address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and nrgc: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to NRGC addresses"));
...
//    messagingAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Nrgc addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Nrgc addresses"));
    ...
    openAction->setStatusTip(tr("Open a nrgc: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Nrgc command-line options").arg(tr(PACKAGE_NAME)));
...
void NrgcGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        if(IconsOnly) {
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/nrgctext")));
        }
        else {
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/nrgccointext")));
        }
...
//        m_toolbar->addAction(votingAction);
        ...
        QVBoxLayout* nrgcLabelLayout = new QVBoxLayout(toolbarWidget);
        nrgcLabelLayout->addWidget(labelToolbar);
        nrgcLabelLayout->addWidget(m_toolbar);
        nrgcLabelLayout->setDirection(QBoxLayout::TopToBottom);
        nrgcLabelLayout->addStretch(1);
        ...
        labelCurrentMarket->setText(tr("Nrgccoin Market Price"));
        ...
        comboNrgcUnit = new QComboBox(headerWidget);
        QStringList list;
        for(int unitNum = 0; unitNum < CurrencyUnits::count(); unitNum++) {
            list.append(QString(CurrencyUnits::CurrencyOptions[unitNum].Header));
        }
        comboNrgcUnit->addItems(list);
        comboNrgcUnit->setFixedHeight(26);
        comboNrgcUnit->setContentsMargins(5,0,0,0);
        comboNrgcUnit->setStyleSheet(STRING_LABEL_COLOR);
        comboNrgcUnit->setFont(currentMarketFont);

        labelVersionUpdate->setText("<a href=\"https://github.com/NrgcProject/Nrgccoin/releases\">New Wallet Version Available</a>");
        ...
        priceLayout->addWidget(comboNrgcUnit, 0 , Qt::AlignBottom| Qt::AlignLeft);
        ...
        connect(comboNrgcUnit, SIGNAL(activated(int)), this, SLOT(currencySelectionChanged(int)));
        // Create the timer
        ...
        // Get the latest Nrgccoin release and let the user know if they are using the latest version
        // Network request code for the header widget
                       ...
                       if (key == "tag_name") {
                           ...
                           if (fNewSoftwareFound) {
                               ...
                               if (GetRandInt(2) == 1) {
                                   ...
                                   if (fRet) {
                                       QString link = "https://github.com/NrgcProject/Nrgccoin/releases";
...
void NrgcGUI::updateIconsOnlyToolbar(bool IconsOnly)
{
    if(IconsOnly) {
        labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/nrgctext")));
        m_toolbar->setMaximumWidth(65);
...
void NrgcGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool NrgcGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool NrgcGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void NrgcGUI::setWalletActionsEnabled(bool enabled)
{
...
void NrgcGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void NrgcGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void NrgcGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void NrgcGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void NrgcGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void NrgcGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void NrgcGUI::openClicked()
{
...
void NrgcGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void NrgcGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void NrgcGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void NrgcGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void NrgcGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void NrgcGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** NRGC START */
void NrgcGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void NrgcGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void NrgcGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void NrgcGUI::gotoRestrictedAssetsPage()
{
...
void NrgcGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Nrgc network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void NrgcGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void NrgcGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void NrgcGUI::updateHeadersSyncProgressLabel()
{
...
void NrgcGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void NrgcGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Nrgc"); // default title
...
void NrgcGUI::changeEvent(QEvent *e)
{
...
void NrgcGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void NrgcGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void NrgcGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "NRGC")
        msg += tr("Amount: %1\n").arg(NrgcUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(NrgcUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void NrgcGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void NrgcGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool NrgcGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool NrgcGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void NrgcGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void NrgcGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void NrgcGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void NrgcGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void NrgcGUI::detectShutdown()
{
...
void NrgcGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void NrgcGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
static bool ThreadSafeMnemonic(NrgcGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void NrgcGUI::subscribeToCoreSignals()
{
...
void NrgcGUI::unsubscribeFromCoreSignals()
{
...
void NrgcGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void NrgcGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<NrgcUnits::Unit> units = NrgcUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const NrgcUnits::Unit unit : units)
    {
    #ifndef QTversionPreFiveEleven
        max_width = qMax(max_width, fm.horizontalAdvance(NrgcUnits::name(unit)));
    #else
        max_width = qMax(max_width, fm.width(NrgcUnits::name(unit)));
    #endif
    }
...
{
    menu = new QMenu(this);
    for (NrgcUnits::Unit u : NrgcUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(NrgcUnits::name(u)), this);
...
{
    setText(NrgcUnits::name(newUnits));
}
...
void NrgcGUI::currencySelectionChanged(int unitIndex)
{
    if(clientModel && clientModel->getOptionsModel())
    {
...
void NrgcGUI::onCurrencyChange(int newIndex)
{
    qDebug() << "NrgcGUI::onPriceUnitChange: " + QString::number(newIndex);
    ...
    this->comboNrgcUnit->setCurrentIndex(newIndex);
    this->getPriceInfo();
}

void NrgcGUI::getPriceInfo()
{
    request->setUrl(QUrl(QString("https://api.coingecko.com/api/v3/simple/price?ids=nrgc&vs_currencies=%1").arg(this->currentPriceDisplay->Ticker)));

    https://api.coingecko.com/api/v3/simple/price?ids=nrgc&vs_currencies=
    networkManager->get(*request);
}

#ifdef ENABLE_WALLET
void NrgcGUI::mnemonic()
{
...
void NrgcGUI::getLatestVersion()
{
    versionRequest->setUrl(QUrl("https://api.github.com/repos/NrgcProject/Nrgccoin/releases"));
