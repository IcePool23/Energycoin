...
#include "nrgcaddressvalidator.h"
#include "nrgcunits.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Nrgc address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(GetParams()))));
#endif
    widget->setValidator(new NrgcAddressEntryValidator(parent));
    widget->setCheckValidator(new NrgcAddressCheckValidator(parent));
}
...
bool parseNrgcURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no nrgc: URI
    if(!uri.isValid() || uri.scheme() != QString("nrgc"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!NrgcUnits::parse(NrgcUnits::NRGC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseNrgcURI(QString uri, SendCoinsRecipient *out)
{
    // Convert nrgc:// to nrgc:
    //
    //    Cannot handle this later, because nrgc:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("nrgc://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "nrgc:");
    }
    QUrl uriInstance(uri);
    return parseNrgcURI(uriInstance, out);
}

QString formatNrgcURI(const SendCoinsRecipient &info)
{
    QString ret = QString("nrgc:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(NrgcUnits::format(NrgcUnits::NRGC, info.amount, false, NrgcUnits::separatorNever));
        paramCount++;
    }
...
bool openNrgcConf()
{
    boost::filesystem::path pathConfig = GetConfigFile(NRGC_CONF_FILENAME);
    ...
    /* Open nrgc.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Nrgc.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Nrgc (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Nrgc (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "nrgc.desktop";
    return GetAutostartDir() / strprintf("nrgc-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a nrgc.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Nrgc\n";
        else
            optionFile << strprintf("Name=Nrgc (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the nrgc app
...
bool GetStartOnSystemStartup()
{
    CFURLRef nrgcAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (nrgcAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, nrgcAppUrl);

    CFRelease(nrgcAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add nrgc app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, nrgcAppUrl, nullptr, nullptr);
    }
