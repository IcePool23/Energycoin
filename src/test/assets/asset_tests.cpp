...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of ENERGYCOIN NOT allowed
        BOOST_CHECK(!IsAssetNameValid("NRGC", type));
        BOOST_CHECK(!IsAssetNameValid("NRGC", type));
        BOOST_CHECK(!IsAssetNameValid("ENERGYCOIN", type));

        //- Versions of ENERGYCOIN ALLOWED
        BOOST_CHECK(IsAssetNameValid("NRGC.COIN", type));
        BOOST_CHECK(IsAssetNameValid("NRGC_COIN", type));
        BOOST_CHECK(IsAssetNameValid("NRGCSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERNRGC", type));
        BOOST_CHECK(IsAssetNameValid("NRGCSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDENRGC", type));
        BOOST_CHECK(IsAssetNameValid("BLACK_NRGCS", type));
        BOOST_CHECK(IsAssetNameValid("SENRGCOT", type));
    ...
    BOOST_AUTO_TEST_CASE(transfer_asset_coin_test)
    {
        ...
        CAssetTransfer asset("NRGC", 1000);
    ...
    BOOST_AUTO_TEST_CASE(new_asset_coin_test)
    {
        ...
        // Create the asset scriptPubKey
        CNewAsset asset("NRGC", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(GetParams().GlobalBurnAddress()));
        asset.ConstructTransaction(scriptPubKey);
