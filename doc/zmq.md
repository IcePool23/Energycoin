...
The Nrgc Core daemon can be configured to act as a trusted "border
router", implementing the nrgc wire protocol and relay, making
...
The ZeroMQ feature in Nrgc Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building nrgcd:
...
For instance:
    ...
    $ nrgcd -zmqpubhashtx=tcp://127.0.0.1:28766 \
               -zmqpubrawtx=ipc:///tmp/nrgcd.tx.raw
...
These options can also be provided in nrgc.conf.
...
From the perspective of nrgcd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into nrgcd directly. Furthermore, no information is
...
using. Nrgcd appends an up-counting sequence number to each
