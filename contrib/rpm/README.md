...
The RPM spec file provided here is for Nrgc-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://nrgc.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Nrgc git repository](https://github.com/NrgcProject/Nrgccoin) but are not part of
...
Currently only `Source10` is used, for the example `nrgc.conf` file.
...
in the nrgc git repository. At present only an SVG version of the Nrgc
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the nrgc
...
needed when 0.12.1 is released, a proper fix is already in the Nrgc git
master branch.
...
idea to build Nrgc Core against a static version of that library compiled
...
The `nrgc.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `nrgc-server` package. At this point, most RPM based
...
The `nrgc-server` package should have SELinux support. How to properly do
...
Nrgc-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/nrgc/). He has been
packaging Nrgc for Fedora at least since 2012.
...
4. I renamed the `nrgc` package that contains the Qt GUI to `nrgc-core` as
that appears to be how the general population refers to it, in contrast to
`nrgc-xt` or `nrgc-classic`. I wanted to make sure the general population
