...
# Copyright (c) 2023-2024 The Nrgc developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Nrgc database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Nrgc datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Nrgc datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Nrgc 0.7 datadir."
    ;;
  3)
    echo "Detected Nrgc pre-0.8 datadir."
    ;;
  4)
    echo "Detected Nrgc 0.8 datadir."
    ;;
esac
