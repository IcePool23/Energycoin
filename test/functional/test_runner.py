...
`test/functional/test_framework/test_framework.py:NrgcTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/nrgc_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    # Check that the build was configured with wallet, utils, and nrgcd
    enable_wallet = config["components"].getboolean("ENABLE_WALLET")
    enable_cli = config["components"].getboolean("ENABLE_UTILS")
    enable_nrgcd = config["components"].getboolean("ENABLE_NRGCD")
    if not (enable_wallet and enable_cli and enable_nrgcd):
        print("No functional tests to run. Wallet, utils, and nrgcd must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, use_term_control, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, last_loop=False):
    # Warn if nrgcd is already running (unix only)
    if args is None:
        args = []
    try:
        if subprocess.check_output(["pidof", "nrgcd"]) is not None:
            print("%sWARNING!%s There is already a nrgcd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "NRGCD" not in os.environ:
        os.environ["NRGCD"] = build_dir + '/src/nrgcd' + exeext
        os.environ["NRGCCLI"] = build_dir + '/src/nrgc-cli' + exeext
...
class RPCCoverage:
    ...
    commands per `nrgc-cli help` (`rpc_interface.txt`).
